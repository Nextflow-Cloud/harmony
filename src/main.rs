...
use services::environment::{LISTEN_ADDRESS, JWT_SECRET};
use services::webrtc;
...
use crate::methods::{RpcApiMethod, Method, RpcApiResponse, Response, NotFoundResponse, HelloEvent, Event, RpcApiEvent};
...
async fn main() {
    ...
    while let Some(stream) = incoming.next().await {
        ...
        spawn (async move {
            ...
            let socket_arc = Arc::new(Mutex::new(accept_async(connection).await.expect("Failed to accept")));
            let id = generate(random_number, &['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'], 10);
                ...
                while let Some(data) = socket.next().await {
                    match data.unwrap() {
                        Message::Binary(bin) => {
                            ...
                            let result: RpcApiMethod = Deserialize::deserialize(&mut deserializer).expect("Failed to deserialize");
                            ...
                            let dispatch: Response = match data {
                                ...
                                _ => {
                                    fn not_found(_: Arc<Mutex<WebSocketStream<TcpStream>>>) -> Response {
                            ...
                            return_value.serialize(&mut Serializer::new(&mut value_buffer)).unwrap();
