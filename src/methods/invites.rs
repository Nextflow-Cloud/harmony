...
use crate::{
    ...
    services::{
        database::{
            channels::{get_channel, Channel},
            infractions::is_banned,
            invites::{create_invite, delete_invite, get_invite, get_invites, Invite},
            members::get_member,
            spaces::get_space,
        },
        permissions::{permissions_for, Permission},
        socket::RpcClient,
    },
};
                ...
                if let Some(space_id) = invite.space_id {
                    ...
                    match space {
                        Ok(space) => {
                            ...
                            match banned {
                                Ok(banned) => Response::GetInvite(GetInviteResponse {
